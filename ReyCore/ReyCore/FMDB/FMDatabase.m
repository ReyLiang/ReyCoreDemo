...
@synthesize versionState=_versionState;
...
- (instancetype)initWithPath:(NSString *)inPath
                     version:(int)version
                 updateBlock:(void(^)(void * fmdatabase , int oldVersion , int newVersion)) updateBlock
{
    ...
    if (self) {
        ...
        _versionState = [self checkUserVersionWithVersion:version updateBlock:updateBlock];
    }
...
#pragma mark Version Check by Rey

/**
 *  sqlite3_exec callback
 *
 *  @param para   version para
 *  @param nCount
 *  @param pValue
 *  @param pName
 *
 *  @return
 */
int userVersion(void* para,int nCount,char** pValue,char** pName)
{
    NSString * version = [NSString stringWithUTF8String:pValue[0]];
    *(int *)para = [version intValue];
...
- (int)getUserVersion
{
    ...
    int version;
    int re = sqlite3_exec(_db, "PRAGMA user_version;", userVersion, &version, 0);
    
    if (re != SQLITE_OK) {
        if (_logsErrors) {
            NSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
            NSLog(@"DB Query: PRAGMA user_version;");
            NSLog(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
            if (_crashOnErrors) {
                abort();
...
- (BOOL)setUserVersion:(int)newVersion
{
    
    NSString * sql = [NSString stringWithFormat:@"PRAGMA user_version = %d;",newVersion];
    int re = sqlite3_exec(_db, [sql UTF8String], NULL, 0, 0);
...
/**
 *  Check sqlite db's user_version value
 *
 *  @param version lastest version or custom version
 *
 *  @return yes => version != [user_version] no => others
 */
- (VersionState)checkUserVersionWithVersion:(int)version
                                updateBlock:(void(^)(void * fmdatabase , int oldVersion , int newVersion)) updateBlock
{
    int oldVersion = [self getUserVersion];
    
    if (-1 == oldVersion) {
        
        return VersionError;
    }
    if ([self getUserVersion] == version) {
        return VersionSame;
    }
    
    [self setUserVersion:version];
    
    if (updateBlock != NULL) {
        updateBlock(self , oldVersion , version);
    }
